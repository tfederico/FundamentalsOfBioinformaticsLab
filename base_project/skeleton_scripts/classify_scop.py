...
def read_lookup_table(filename):
    ...
    More about PDB https://en.wikipedia.org/wiki/Protein_Data_Bank
    """
    ##########################
    ### START CODING HERE ####
    ##########################
...
def main(uniprot_ids_file_name, output_file_name, lookup_file_name, scop_file_name):
    ...
    #conversion_dictionary is needed to convert Uniprot IDs to Protein Data Bank (PDB) IDs
    conversion_dictionary = read_lookup_table(lookup_file_name) # dic =  { "uniprotID": pdbID}

    uniprot_ids = read_protein_ids_file(uniprot_ids_file_name)
